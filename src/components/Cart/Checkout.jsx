...
const Checkout = () => {
  const [paymentMethod, setPaymentMethod] = useState("shiprocket");
  ...
  useEffect(() => {
    ...
    syncCart();
  }, [dispatch, navigate, user, guestId, checkoutTraceId]);
        ...
        <form onSubmit={handleCreateCheckout}>
          <div className="space-y-4">
            ...
            <input
              type="email"
              value={user?.email || ""}
              className="w-full p-3 border border-gray-200 bg-gray-50 focus:outline-none focus:border-lv-gold"
              disabled
            />
          ...
          <div className="mt-6">
            ...
            <div className="space-y-3">
              ...
              <button
                type="button"
                onClick={() => {
                  setPaymentMethod("shiprocket");
                  setPaymentError("");
                }}
                className={`w-full rounded-md border px-4 py-3 text-left transition-colors ${paymentMethod === "shiprocket" ? "border-[#2874f0] bg-[#f0f5ff]" : "border-gray-300 bg-white hover:border-[#2874f0]/60"}`}
              >
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-sm font-semibold text-gray-900">Online Payment (OTP Checkout)</p>
                    <p className="text-xs text-gray-600">Verify mobile and pay securely via UPI, cards, wallets, or netbanking.</p>
                  </div>
                  <span className={`inline-flex h-5 w-5 items-center justify-center rounded-full border ${paymentMethod === "shiprocket" ? "border-[#2874f0] bg-[#2874f0] text-white" : "border-gray-300 bg-white text-transparent"}`}>
                    •
                  </span>
                </div>
              </button>
            ...
            {paymentMethod === "shiprocket" && (
              <div className="mt-3 rounded-md border border-[#dfe6f5] bg-[#f7f9ff] px-4 py-3">
                <p className="text-xs font-semibold uppercase tracking-[0.08em] text-[#1f5fd1]">
                  Secure Hosted Checkout
                </p>
                <p className="mt-2 text-[11px] text-gray-600 leading-relaxed">
                  You will continue to Shiprocket Checkout for mobile OTP verification and payment. After successful payment,
            ...
            {paymentMethod === "COD" && (
              <p className="mt-3 text-xs text-amber-700">
                COD is selected. Switch to Online Payment to open the Shiprocket payment page.
              </p>
            )}
