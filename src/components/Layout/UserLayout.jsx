...
const UserLayout = ({ isVisible }) => {
  ...
  return (
    <div className="overflow-x-hidden">
        ...
        {/* Bottom layout */}
        <div className="fixed bottom-0 left-0 right-0 bg-black text-white shadow-lg flex justify-around items-center p-4 md:hidden">
          <Link to="/" className="flex flex-col items-center">
            <i className="fas fa-home w-15 h-15" />
          </Link>
          <Link to="/community" className="flex flex-col items-center">
            <i className="fas fa-comments w-15 h-15" />
          </Link>
          <Link to="/profile" className="flex flex-col items-center">
            <i className="fas fa-user w-15 h-15" />
          </Link>
          <a href="https://wa.me/917460935762" target="_blank" rel="noopener noreferrer" className="flex flex-col items-center">
            <i className="fab fa-whatsapp w-15 h-15" />
          </a>
        </div>
    </div>
  );
