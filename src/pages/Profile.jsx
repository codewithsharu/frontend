...
const PROFILE_MAX_SIZE_BYTES = 3 * 1024 * 1024;
...
const Profile = () => {
  ...
  const handleProfileImageChange = (e) => {
    ...
    if (file.size > PROFILE_MAX_SIZE_BYTES) {
      setProfileFeedback({
        type: "error",
        message: "Profile image is too large. Maximum size is 3MB.",
      });
      e.target.value = "";
      return;
    }
  ...
  const sectionLinkClass = (key) => {
    ...
    return `group flex items-center gap-3 rounded-lg border px-3 py-3 text-left transition-colors ${
      isActive
        ? "border-[#2874f0] bg-[#2874f0] text-white shadow-sm"
        : "border-gray-200 bg-white text-[#212121] hover:border-[#2874f0]/50"
    }`;
  };
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-[300px_1fr] lg:gap-6">
          <aside className="rounded-lg border border-gray-200 bg-white shadow-sm">
            <div className="border-b border-gray-100 p-4 sm:p-5">
              <div className="flex items-center gap-3">
                <div className="flex h-11 w-11 items-center justify-center rounded-full bg-[#2874f0] text-sm font-bold text-white">
                    ...
                    >
                      <span
                        className={`inline-flex h-9 w-9 items-center justify-center rounded-md ${
                          isActive ? "bg-white/20" : "bg-[#f5f7fa] text-[#2874f0]"
                        }`}
                      >
          ...
          {activeSection === "account" && (
            <div className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm sm:p-6">
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#2874f0]">Account Overview</p>
              ...
              <form onSubmit={handleSaveProfile} className="mt-5 rounded-lg border border-gray-200 p-4 sm:p-5">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                  <div className="flex items-center gap-3">
                    <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-full border border-gray-200 bg-[#f5f7fa] text-lg font-semibold text-[#2874f0]">
                      {displayedProfileImage ? (
                    ...
                    <div>
                      <label
                        ...
                        className="inline-flex cursor-pointer items-center gap-2 rounded-md border border-gray-300 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.08em] text-[#212121] hover:border-[#2874f0] hover:text-[#2874f0]"
                      >
                      ...
                      />
                      <p className="mt-1 text-[11px] text-[#878787]">JPEG, PNG, WebP, AVIF • Max 3MB</p>
                    </div>
                  </div>
                  ...
                  <div className="grid flex-1 grid-cols-1 gap-3 sm:grid-cols-2">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm text-[#212121] focus:outline-none focus:border-[#2874f0]"
                      />
                    </div>
                    <div>
                      <label className="mb-1 block text-[11px] font-semibold uppercase tracking-[0.08em] text-[#878787]">Email</label>
                      <input
                        type="text"
                        value={user?.email || ""}
                        readOnly
                ...
                <div className="mt-4 flex items-center gap-2">
                  <button
                    ...
                    className="rounded-md bg-[#2874f0] px-4 py-2 text-xs font-semibold uppercase tracking-[0.08em] text-white hover:bg-[#1d63d6] disabled:opacity-60"
                  >
              ...
              <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
                <Link
                  ...
                  className="group rounded-lg border border-gray-200 p-4 transition-colors hover:border-[#2874f0]/50"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <span className="inline-flex h-10 w-10 items-center justify-center rounded-md bg-[#f5f7fa] text-[#2874f0]">
                        <FiMapPin />
                      </span>
                      <div>
                    ...
                    <FiChevronRight className="text-[#b0b0b0] transition-colors group-hover:text-[#2874f0]" />
                  </div>
                </Link>
                <Link
                  to="/profile/orders"
          ...
          {activeSection === "addresses" && (
            <div className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm sm:p-6">
              <div className="flex items-start justify-between gap-3 mb-4">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#2874f0]">Address Book</p>
                ...
                <button
                  ...
                  className="rounded-md border border-gray-300 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.06em] text-[#4d4d4d] hover:border-[#2874f0] hover:text-[#2874f0]"
                >
                  Refresh
                </button>
              </div>
              ...
              {loadingAddresses ? (
                ...
                <div className="space-y-3 mb-5">
                  {savedAddresses.map((address) => (
                    <div key={address._id} className="rounded-lg border border-gray-200 bg-white p-4 transition-colors hover:border-[#2874f0]/40">
                      <div className="flex items-center justify-between gap-3">
                        <p className="text-sm font-semibold text-[#212121]">
                          {(address.firstName || "").trim()} {(address.lastName || "").trim()}
                        </p>
                        <span className="rounded-full border border-[#2874f0]/30 bg-[#f0f5ff] px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.06em] text-[#2874f0]">
                          {address.label || "Saved"}
                        </span>
                      </div>
                      {address.isDefault && (
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {!address.isDefault && (
                          <button
                            ...
                            className="rounded-md border border-[#2874f0] bg-white px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.06em] text-[#2874f0] hover:bg-[#f0f5ff] disabled:opacity-60"
                          >
                            Set Default
                          </button>
                        )}
                        <button
                          ...
                          className="rounded-md border border-gray-300 bg-white px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.06em] text-gray-700 hover:border-[#2874f0] hover:text-[#2874f0]"
                        >
                          Edit
                        </button>
                        <button
                ...
                <form onSubmit={handleSaveAddress} className="space-y-3">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <input
                      ...
                      className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:border-[#2874f0]"
                    />
                    <input
                      type="text"
